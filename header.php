...
<script>function gtag(){dataLayer.push(arguments)}window.dataLayer=window.dataLayer||[],gtag("js",new Date),gtag("config","<?php echo get_option('tracking_ga'); ?>");</script>
<?php } ?>
...
<div id="side-menu">
    ...
    if ((int) get_option('reusable_block_mobile_id') > 0) {
        $reusableBlockMobileId = get_post((int) get_option('reusable_block_mobile_id'));

        echo '<div class="mobile-menu--inner">';
            echo apply_filters('the_content', $reusableBlockMobileId->post_content);
        echo '</div>';
    }
    ?>
</div>
